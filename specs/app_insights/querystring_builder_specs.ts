...
describe('AppInsightsQuerystringBuilder',() => {
  ...
  describe('with filter on segment', () => {
    beforeEach(function() {
      builder.setFilter('request/urlHost', 'eq', 'fabrikamfiberapp.azurewebsites.net');
    });

    it('should add filter on to querystring',() => {
      const querystring = `timespan=2017-08-22T06:00:00Z/2017-08-22T07:00:00Z` +
      `&filter=request/urlHost eq 'fabrikamfiberapp.azurewebsites.net'`;
      expect(builder.generate()).to.equal(querystring);
    });
  });
});
